...
class Job
{
    ...
    public function __construct($command, array $args = [], $confirmed = true, $queue = self::DEFAULT_QUEUE, $priority = self::PRIORITY_DEFAULT)
    {
        ...
        if (mb_strlen($queue) > self::MAX_QUEUE_LENGTH) {
            throw new \InvalidArgumentException(sprintf('The maximum queue length is %d, but got "%s" (%d chars).', self::MAX_QUEUE_LENGTH, $queue, mb_strlen($queue)));
        }
    ...
    public function isStartable()
    {
        foreach ($this->dependencies as $dep) {
            if (self::STATE_FINISHED !== $dep->getState()) {
                return false;
            }
        }
