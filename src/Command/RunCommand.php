...
class RunCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if (mb_strlen($workerName) > 50) {
            throw new \RuntimeException(sprintf('"worker-name" must not be longer than 50 chars, but got "%s" (%d chars).', $workerName, mb_strlen($workerName)));
        }
    ...
    private function checkRunningJobs()
    {
        foreach ($this->runningJobs as $i => &$data) {
            $newOutput = mb_substr($data['process']->getOutput(), $data['output_pointer']);
            $data['output_pointer'] += mb_strlen($newOutput);

            $newErrorOutput = mb_substr($data['process']->getErrorOutput(), $data['error_output_pointer']);
            $data['error_output_pointer'] += mb_strlen($newErrorOutput);
